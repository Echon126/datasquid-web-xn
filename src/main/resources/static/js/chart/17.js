...
webSquid.page.modules["chart/17"] = webSquid.page.modules["chart/17"] || {
        ...
        graph: function () {
            ...
            webSquid.ajax("data/s/HJ", "GET", {
                sid: "c17_barlinetable",
                ie: webSquid.ie,
                sign: webSquid.sign,
                start: start,
                end: end
            }, function (rsp) {
                var m = webSquid.pageModule("chart/17");
                // 清空柱状图数据
                m.bar_data = [];
                m.bar_data_x = [];
                // 清空柱线图数据
                ...
                // 表格赋值
                webSquid.datatable("#query-c17-table", {
                    paging: false,
                    ordering: false,
                    info: false,
                    filter: false,
                    data: webSquid.pageModule("chart/17").table_data,
                    // order: [0, 'asc'],
                    columns: [
                        {data: "cycle", label: "周期"},
